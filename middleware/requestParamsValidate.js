...
const requestParamsValidate = (schema) => {
    return (req, res, next) => {
        ...
        const queryResult = querySchema ? querySchema.validate(queryData, options) : { error: null };
        const bodyResult = bodySchema ? bodySchema.validate(bodyData, options) : { error: null };

        if (queryResult.error || bodyResult.error) {
            const queryError = queryResult.error ? queryResult.error.details.map(i => i.message).join(',') : '';
